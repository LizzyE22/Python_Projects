...
def create_db(self):
    ...
    with conn:
        cur = conn.cursor()
        # this creates db, and all the fields and rows
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,_col_email) VALUES (?,?,?,?,?)""",
            conn.commit()
    conn.close()
...
def onSelect(self, event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email FROM tbl_phonebook WHERE col_fullname = (?)""", [value1])
...
def addToList(self)
    ...
    var_lname = var_lname.strip() # This will ensure the first character in each word is capitalized
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname))# combine our normalized name into a fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #enforce the user to provide
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))#,(var_full)
            count = cursor.fetchone() [0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existence of the full name and we can add new data
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FRPM tbl_phonebook WHERE col_fullname = '{}'""".format(var_select))
                onDeleted(self) # call the function to clear all of the textboxes and the selected index of listbox
######              onRefresh(self) # update the listbox of the changes
                conn.commit()
        else:
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at this t)
    conn.close()
...
def OnClear(self):
    # clear the text on these textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
...
def onRefresh(self):
    # Poopulate the listbox, coinnciding with the database
    self.listList1.delete(0,END)
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cursor = conn.cursor()
...
def onUpdate(self):
    try:
        ...
        messagebox.showinfo("Missing selection","No name was selected from the list box. \Cancelling the Update request.")
        return
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    messagebox.shownifo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
