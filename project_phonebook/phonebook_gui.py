...
def load_gui(self):
    ...
    self.txt_email.grid(row=7,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+W)

    # Define the listbox with a scrollbar and grid them

    self.scrollbar1 = Scrollbar(self.master,orient=VERTICAL)
    self.lst.list1 = Listbox(self.master,exportselection=0,yscrollcommand=self.scrollbar1.set)
